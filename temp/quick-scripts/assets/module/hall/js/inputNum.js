...
cc.Class({
    ...
    * 房间号监听输入
    */
    clickNum: function clickNum(event) {
    ...
    */
    ...
    //删除按钮
    removeOneClick: function removeOneClick() {
        if (array != "") {
            if (array.length == 1) {
                this.inputNum1.string = "";
            } else if (array.length == 2) {
    ...
    jjroom: function jjroom(event) {
        var type = event.target.name;
        var arry = event.target.children[0].getComponent(cc.Label).string.split(' ');
    ...
    jjsucess: function jjsucess(result, object) {
        console.log(result);
    ...
    jjerror: function jjerror(result, object) {
        object.closeloadding();
        object.alert2('比赛未开始或者您没资格进入比赛');
    }
});
...
cc._RF.pop();
        ...
        //# sourceMappingURL=inputNum.js.map
