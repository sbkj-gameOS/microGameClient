(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/module/majiang/js/MJDataBind.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, '8bd7eb6ZplBxIx2msga8LgN', 'MJDataBind', __filename);
// module/majiang/js/MJDataBind.js
...
cc.Class((_cc$Class = {
    ...
    onLoad: function onLoad() {
            ...
            if (cc.sys.localStorage.getItem('take') == 'true') {
                ...
                if (card != null) {
                    ...
                    if (cc.sys.localStorage.getItem('ting') == 'true') {
                        ...
                        var _socket2 = self.getSelf().socket();
                        cc.sys.localStorage.removeItem('ting');
                        _socket2.emit("selectaction", JSON.stringify({
                            action: "ting",
                            actionCard: [card_script.value]
                        }));
                        self.getSelf().tingAction();
                    } else {
                        _socket.emit("doplaycards", card_script.value);
                    }
                    //cc.find("");
        ...
        this.node.on("dan", function (event) {
            ...
            if (context.dans && context.dans.length > 1) {
                ...
                var _socket3 = self.getSelf().socket();
                var danParam = [];
                if (context.dans) {
                    danParam = context.dans[0];
                }
                _socket3.emit("selectaction", JSON.stringify({
                    action: 'dan',
                    actionCard: danParam
                }));
            }
        ...
        this.node.on("gang", function (event) {
            ...
            if (context.gangs && context.gangs.length > 1) {
                ...
                var _socket4 = self.getSelf().socket();
                var gangParam = [];
                if (context.gangs) {
                    gangParam = context.gangs[0];
                }
                _socket4.emit("selectaction", JSON.stringify({
                    action: 'gang',
        ...
        this.node.on("chi", function (event) {
            ...
            if (context.chis && context.chis.length > 1) {
                ...
                var _socket5 = self.getSelf().socket();
                _socket5.emit("selectaction", JSON.stringify({
                    action: 'chi',
        ...
        this.node.on("guo", function (event) {
            ...
            if (cc.sys.localStorage.getItem('guo') != 'true' || cc.sys.localStorage.getItem('alting') == 'true') {
                ...
                var _socket6 = self.getSelf().socket();
                _socket6.emit("selectaction", JSON.stringify({
                    action: "guo",
                    actionCard: []
                }));
            } else {
    ...
    shouOperationMune: function shouOperationMune() {
    ...
    // 初始化房间信息
    playerIsReady: function playerIsReady() {
        // // 游戏logo
        ...
        if (cc.weijifen.match == 'false') {
            var roomNum = cc.find('Canvas/roomNum').getChildByName('room')._components[0]; // roomNum节点
    ...
    * 初始化对象池
    */
    init_pool: function init_pool(context) {
        /**
    ...
    joinRoom: function joinRoom() {
        ...
        if (cc.weijifen.room) {
            ...
            param.playway = '402888815e6f0177015e71529f3a0001', param.match = 1;
        }
            ...
            case "nextplayer":
                if (object.action) {
                    if (object.action == "two") {
                        ...
                        var ani = object.actionnode_three.getComponent(cc.Animation);
    ...
    mjOperation: function mjOperation(event, params, context) {
    ...
    readyNoActive: function readyNoActive(context) {
...
}, _defineProperty(_cc$Class, 'exchange_searchlight', function exchange_searchlight(direction, context) {
    cc.sys.localStorage.removeItem('cl');
    context = cc.find('Canvas').getComponent('MJDataBind');
    for (var inx = 0; inx < context.searchlight.children.length; inx++) {
        if (direction == context.searchlight.children[inx].name) {
...
}), _defineProperty(_cc$Class, 'tingAction', function tingAction(dd) {
    var length = cc.find('Canvas/cards/handcards/current/currenthandcards').children.length;
    for (var i = 0; i < length; i++) {
        var cards = cc.find('Canvas/cards/handcards/current/currenthandcards').children[i];
        var button = cc.find('Canvas/cards/handcards/current/currenthandcards').children[i].children[0];
...
cc._RF.pop();
        ...
        //# sourceMappingURL=MJDataBind.js.map
