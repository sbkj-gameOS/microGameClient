(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/module/majiang/js/gameEvent/GameEvent.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, '67bbeo3e7VKwKjcTp+brx4g', 'GameEvent', __filename);
// module/majiang/js/gameEvent/GameEvent.js
    ...
    statics: {
        ...
        action_event: function action_event(data, context) {
            ...
            if (data.chis) {
                ...
                for (var _i = 0; _i < data.chis.length; _i++) {
                    context.chis[_i].push(data.card);
                }
            } else {
                context.chis = [];
            }
            ...
            if (data.deal == true) {
                ...
                for (var inx = 0; inx < gameEventNode.actionnode_two_list.children.length; inx++) {
                    var temp = gameEventNode.actionnode_two_list.children[inx];
                    if (temp.name == "gang") {
                        gang = temp;
                    }
                    if (temp.name == "dan") {
                ...
                for (var inx = 0; inx < gameEventNode.actionnode_two_list.children.length; inx++) {
                    var _temp = gameEventNode.actionnode_two_list.children[inx];
                    if (_temp.name == "gang") {
                        gang = _temp;
                    }
                    if (_temp.name == "peng") {
        ...
        selectaction_event: function selectaction_event(data, context) {
            ...
            if (cc.sys.localStorage.getItem('cb') != 'true' && (data.action == 'peng' || data.action == 'gang' && data.card != -1 || data.action == 'chi' || data.action == 'hu')) {
                ...
                var temp = gameStartInit.player(data.target, context),
                    deskcardpanel = null;
        ...
        handCardRemove: function handCardRemove(data, context) {
            if (data.cards) {
                for (var _i2 = 0; _i2 < data.cards.length; _i2++) {
                    for (var _inx = 0; _inx < context.playercards.length; _inx++) {
                        var temp = context.playercards[_inx].getComponent("HandCards");
                        if (data.cards[_i2] == temp.value) {
                            context.cardpool.put(context.playercards[_inx]);
                            context.playercards.splice(_inx, 1);
                        }
                    }
                }
            }
            if (data.card != -1) {
                for (var inx = 0; inx < context.playercards.length;) {
                    var _temp2 = context.playercards[inx].getComponent("HandCards");
                    if (data.card == _temp2.value) {
        ...
        otherHandCardRemove: function otherHandCardRemove(data, context, tablepos) {
            var gameStartInit = cc.find('Canvas/js/GameStartInit').getComponent('GameStartInit');
            for (var _i3 = 0; _i3 < data.cards.length; _i3++) {
                if (tablepos == 'top') {
                    gameStartInit.top_panel.children[_i3].destroy();
                    context.topcards.splice(_i3, 1);
                } else if (tablepos == 'right') {
                    gameStartInit.right_panel.children[_i3].destroy();
                    context.rightcards.splice(_i3, 1);
                } else if (tablepos == 'left') {
                    gameStartInit.left_panel.children[_i3].destroy();
                    context.leftcards.splice(_i3, 1);
                }
            }
        },
        cardModle: function cardModle(cards, parent, back, fangwei, context, action, target) {
            var gameStartInit = require('GameStartInit');
            var gameEventNode = cc.find('Canvas/js/GameEvent').getComponent('GameEvent');
            if (cards.length == 1) {
                ...
                for (var _i4 = 0; _i4 < cards.length; _i4++) {
                    ...
                    if (_i4 == 2 && back == true) {
                        temp.init(cards[_i4], false, fangwei, '1');
                    } else if (action != 'dan' && _i4 == 1 && back != true) {
                        temp.init(cards[_i4], false, fangwei, '1', target);
                    } else {
                        temp.init(cards[_i4], back, fangwei, '1');
                    }
                    if (71 < cards[_i4] && cards[_i4] < 76 && action != 'chi') {
                        card.zIndex = 9999;
                    } else {
                        card.zIndex = 0;
                    }
        ...
        findCardForKong: function findCardForKong(kong, card, action) {
            ...
            var cardNum;
            //遍历整个kong 的子集  cards、
            for (var _i5 = 0; _i5 < kong.children.length; _i5++) {
                var cards = kong.children[_i5];
                ...
                if (cardtype == 2 && parseInt(card % 36 / 4) == 0 && cards.children.length > 0 && type != 'yao' && action == 'dan' && kaction == 'dan') {
                    ...
                    if (action == 'gang' && dans.length > 0 & kaction == 'peng') {
                        ...
                        for (var _j = 0; _j < dans.length; _j++) {

                            var cardUnit = dans[_j];
